...
function OutputContent({
  ...
  if (activeOutputTab === "stdout") {
    if (!execution.output.stdout) {
      return <span className="text-[14px] italic text-[var(--text-dim)]">empty</span>;
    }
...
export function IOPanels({
  ...
  return (
    <div className="grid min-h-0 overflow-hidden border-t border-[var(--border)] grid-cols-[minmax(225px,1fr)_minmax(0,1.4fr)]">
      <div className="flex min-h-0 flex-col overflow-hidden border-r border-[var(--border)]">
        <div className="flex shrink-0 items-center gap-2 border-b border-[var(--border)] bg-[var(--bg3)] px-3 py-1 text-[12px] font-bold tracking-[0.06em] text-[var(--text-dim)] uppercase">
          <ArrowRight size={12} strokeWidth={2} />
          stdin
        </div>
        <div className="min-h-0 flex-1 overflow-auto px-3 py-2 text-[15px] leading-[1.5] [scrollbar-color:var(--bg4)_transparent] [scrollbar-width:thin]">
          <textarea
            className="h-full w-full resize-none border-0 bg-transparent font-[var(--font-mono)] text-[15px] leading-[1.5] text-[var(--text)] outline-none"
            spellCheck={false}
            value={input}
